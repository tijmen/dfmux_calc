...
class experiment:
    def __init__(self,name,dfmux_noise):
        if name == 'litebird':
            ...
            self.popt = np.array([0.29184686,0.24187821,0.26855768,0.30598544,0.27094372,0.2958127,0.3278966,0.31625242,0.37653296,0.32719336,
                         0.30607807,0.35566829,0.35588565,0.43860747,0.42063367,0.39077456,0.35717258,0.62886291,0.4708167,0.37702823,
                         0.2996769,0.22026075]) * 1e-12
            
            #assumes that the saturation power of a liteBIRD bolometer is tuned to be 2.5 times the optical power in the band
            self.psat = np.array([ 2.5 * popt for popt in self.popt])
            
            #requirements on readout noise in aW/rtHz in each band
            self.rreq = np.array([2.67399478,2.43434099,2.5650854,2.73799985,2.57645516,2.69210163,2.83433708,
                         2.78355604,3.03727798,2.83129603,2.73841425,2.95192701,2.95282887,3.27809138,3.21022195,3.09418433,
                         2.95816298,3.92519316,3.39632288,3.03927487,2.70962803,2.32301277]) *1e-18
            self.ntot = np.array([8.31475732,7.83616637,8.64772011,8.63963629,8.50986423,9.36124323,9.5290737,9.72258017,11.27231753,9.69276546,9.77778099,
                         11.44233862,10.64069174,12.25155928,12.51592665,12.67883396,12.77011888,17.19246695,15.76686961,15.03623678,
                         14.41789678,13.34004439]) * 1e-18
            self.nphon = np.array([5.44441647,5.26100438,5.97783229,5.71954248,5.81005536,6.58377923,6.57637918,6.85404996,8.18115868,6.75923232,
                        6.97036698,8.45472357,7.61933154,8.9189256,9.26864051,9.55735219,9.78367735,13.22647082,12.31481045,11.91261558,
                          11.58180607,10.84704975]) * 1e-18
            self.nphot = np.array([3.94755931,3.59376372,3.7867788,4.04204858,3.80356373,3.97429005,4.1842691,4.10930217,4.48386626,
                          4.17977967,4.04266035,4.35786451,4.35919591,4.83937375,4.73917962,4.56787585,4.36707054,
                          5.7946758,5.0139163,4.48681422,4.00016388,3.42941234] )* 1e-18
            ...
            self.band_num = len(self.opt_freqs)
    ...
    def get_r_reqs(self,loopgain,rs,rb):
        for i in range(self.band_num):
            nep = self.ntot[i] * np.sqrt(1.1**2 - 1)
            
            rbolo, rstray = np.meshgrid(np.linspace(1e-3, rb , 100), np.linspace(0, rs, 100))
    
            vbias = np.sqrt( (rbolo + rstray) * (self.psat[i] - self.popt[i]) )
            ...
            ax.set_title('NEI requirement for {} GHz band $P_{{sat}}$={}, $P_{{opt}}=${}, $\mathcal{{L}}=${}, NEP$_{{read}}$={}'.format(
                self.opt_freqs[i], round(self.psat[i]*1e12,2), round(self.popt[i]*1e12,2), loopgain, round(nep*1e18,1)))
