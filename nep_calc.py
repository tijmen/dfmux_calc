...
class experiment:
    ...
    def get_r_reqs(self, loopgain, rs, rb):
        for i in range(self.band_num):
            ...
            rbolo, rstray = np.meshgrid(
                np.linspace(1e-3, rb, 100), np.linspace(0, rs, 100)
            )
            ...
            resp = (
                ...
                / vbias
                * loopgain
                / (1 + loopgain * (rbolo - rstray) / (rbolo + rstray))
            )
            ...
            c = ax.pcolormesh(
                rbolo, rstray, nei_req * 1e12, cmap="jet", vmin=5, vmax=15
            )
    ...
    def nei_to_nep(self, i, err=0):
        ...
        vbias = np.sqrt(
            (self.readout.bolo.r + self.readout.bolo.rstray)
            * (self.psat[i] * (1 + err / 10) - self.popt[i])
        )

        # amount the loopgain is attenuated by
        loop_atten = (self.readout.bolo.r - self.readout.bolo.rstray) / (
            self.readout.bolo.r + self.readout.bolo.rstray
        )
