...
saa.inoise = norm.ppf(
    np.abs(1 - seed), loc=saa.inoise / np.sqrt(nsqratio), scale=nei_spread
)
...
plt.hist2d(
    np.array([bias_f for i in range(100)]).flatten(),
    dfm.total.flatten() * 1e12,
    cmap="Blues",
)
...
plt.title(
    "Estimated noise for " + str(n_sq_draw) + " SAA w/" + str(len(bias_f)) + " ch each"
)
...
for band in range(len(opts)):
    ...
    plt.hist(
        nep_dat.flatten(),
        alpha=0.5,
        bins=np.linspace(np.min(nep_dat), np.max(nep_dat), 20),
    )
    plt.xlabel("Readout NEP [aW/rtHz]")
    plt.title(title + "Band " + str(litebirb.opt_freqs[band]) + " GHz")
    plt.savefig(
        path + "/band_" + str(band).zfill(2) + "_nsq_" + str(nsq) + "_nep_hist.png"
    )
    plt.close()

    np.save(
        path + "/band_" + str(band).zfill(2) + "_nsq_" + str(nsq) + "_nep_dat.npy",
        nep_dat,
    )
