...
tracker_status = 0
print(
    "Calculating needed SQUIDs at {} bands, for {} bolometer resistances, and {} stray resistances for a total of {} scenarios".format(
        len(bands), 10, 10, tracker_total
    )
)
print(
    "... {}% complete".format(round(tracker_status / tracker_total * 100)),
    end="",
    flush=True,
)
...
for band in bands:
    ...
    nep = np.sqrt(lb.nphon[band] ** 2 + lb.nphot[band] ** 2) * np.sqrt(
        (1 + frac) ** 2 - 1
    )

    # define operating bolometer resistance and stray to sweep through
    rbolo, rstray = np.meshgrid(
        np.linspace(rbolo_min, rbolo_max, r_steps),
        np.linspace(rstray_min, rstray_max, r_steps),
    )
    ...
    resp = (
        ...
        * bolo.loopgain
        * loop_atten
        / (1 + bolo.loopgain * loop_atten * (rbolo - rstray) / (rbolo + rstray))
    )
    ...
    for i in range(10):
        for j in range(10):
            ...
            while True:
                ...
                dfm.init_freq(
                    [np.max(bias_f)], skip_spice=skip_spice, csf_factor=csf_factor
                )
                if max(dfm.total) <= target:
                    ...
                    if [i, j] == [itarget, jtarget]:
                        dfm.init_freq(
                            bias_f, skip_spice=skip_spice, csf_factor=csf_factor
                        )
                        fig, ax = plt.subplots()
                        d.plot_noise(dfm, bias_f, "#1f77b4")
                        plt.plot(
                            [np.min(bias_f) / 1e6, np.max(bias_f) / 1e6],
                            [target * 1e12, target * 1e12],
                            "--",
                            label="NEI Requirement",
                            c="#ff7f0e",
                        )
                        ax.set_title(
                            "Readout NEI for {} GHz band $R_{{bolo}}$={}$\Omega$ $R{{stray}}$={}$\Omega$, \n $\mathcal{{L}}=${}, NEP$_{{read}}$={}aW$/\sqrt{{\mathrm{{Hz}}}}$, {}% NEP increase".format(
                                lb.opt_freqs[band],
                                round(dfm.bolo.r[0], 2),
                                round(dfm.bolo.rstray[0], 2),
                                round(bolo.loopgain[0] * loop_atten[i][j], 1),
                                round(nep * 1e18, 1),
                                frac * 100,
                            )
                        )
                        plt.legend()
                        plt.savefig(
                            path + "/branch_band_" + str(band) + "_readout_nei.png"
                        )
                        plt.close()

                        fig, ax = plt.subplots()
                        plt.plot(
                            [f / 1e6 for f in bias_f],
                            1 / dfm.tf.flatten(),
                            label="1/$\chi_{SQ}$",
                        )
                        plt.plot(
                        ...
                        ax.set_title(
                            "$\chi_{{SQ}}$ and $\chi_{{CS}}$ for {} GHz band $R_{{bolo}}$={}$\Omega$ $R{{stray}}$={}$\Omega$, \n $\mathcal{{L}}=${}, NEP$_{{read}}$={}aW$/\sqrt{{\mathrm{{Hz}}}}$, {}% NEP increase".format(
                                lb.opt_freqs[band],
                                round(dfm.bolo.r[0], 2),
                                round(dfm.bolo.rstray[0], 2),
                                round(bolo.loopgain[0] * loop_atten[i][j], 1),
                        ...
                        with open(
                            path + "/branch_band_" + str(band) + "_nc_note.txt", "w"
                        ) as f:
                            # print(dfm.total[-1][-1], dfm.warm_noise_nc[-1])
                            f.write(
                                "at "
                                + str(round(bias_f[-1] / 1e6, 2))
                                + " MHz removing all nuller/carrier noise would lower the total noise by "
                                + str(
                                    round(
                                        np.sqrt(
                                            dfm.total[-1][-1] ** 2
                                            - dfm.warm_noise_nc[-1] ** 2
                                        )
                                        / dfm.total[-1][-1]
                                        * 100,
                                        2,
                    ...
                    if n_sq <= 200:
                        ...
                        if [i, j] == [itarget, jtarget]:
                            ...
                            plt.legend()
                            plt.savefig(
                                path + "/branch_band_" + str(band) + "_readout_nei.svg",
                                format="svg",
                            )
                            plt.close()
                            ...
                            plt.xlabel("Bias frequency [MHz]")
                            ax.set_title(
                                "TF + CS for {} GHz band $R_{{bolo}}$={}$\Omega$ $R{{stray}}$={}$\Omega$, \n $\mathcal{{L}}=${}, NEP$_{{read}}$={}aW$/\sqrt{{\mathrm{{Hz}}}}$, {}% NEP increase".format(
                                    lb.opt_freqs[band],
                                    round(dfm.bolo.r[0], 2),
                                    round(dfm.bolo.rstray[0], 2),
                                    round(bolo.loopgain[0] * loop_atten[i][j], 1),
                            ...
                            plt.legend()
                            plt.savefig(
                                path + "/branch_band_" + str(band) + "_tf_cs.svg",
                                format="svg",
                            )
                            plt.close()
            ...
            print(
                "\r... {}% complete".format(
                    round(tracker_status / tracker_total * 100)
                ),
                end="",
                flush=True,
            )
    ...
    c = ax.pcolormesh(
        rbolo, rstray, req_nsq, cmap="jet", vmin=1, vmax=80, shading="auto"
    )
    c2 = ax.pcolormesh(
        rbolo,
        rstray,
        np.where(fail == 1, 1, np.nan),
        cmap="binary",
        vmin=0,
        vmax=1,
        zorder=10,
    ...
    ax.set_title(
        "# SQ requirement for {} GHz band: $P_{{sat}}$={}pW, $P_{{opt}}=${}pW, \n $\mathcal{{L}}=${}, NEP$_{{read}}$={}aW$/\sqrt{{\mathrm{{Hz}}}}$, {}% NEP increase".format(
            lb.opt_freqs[band],
            round(psat * 1e12, 2),
            round(popt * 1e12, 2),
            bolo.loopgain[0],
    ...
    c = ax.pcolormesh(
        rbolo, rstray, nei_req * 1e12, cmap="jet", vmin=5, vmax=15, shading="auto"
    )
    # d = ax.pcolormesh(rbolo, rstray, np.where(fail == 1, 1, np.nan), cmap='binary', vmin=0, vmax=1,zorder=10)
    CS = ax.contour(rbolo, rstray, loop_atten, 6, colors="w")
    ax.clabel(CS, fontsize=9, inline=True)
    ax.set_title(
        "NEI requirement for {} GHz band: $P_{{sat}}$={}pW, $P_{{opt}}=${}pW, \n $\mathcal{{L}}=${}, NEP$_{{read}}$={}aW$/\sqrt{{\mathrm{{Hz}}}}$, {}% NEP increase".format(
            lb.opt_freqs[band],
            round(psat * 1e12, 2),
            round(popt * 1e12, 2),
            bolo.loopgain[0],
    ...
    ax.clabel(CS, fontsize=9, inline=True)
    ax.set_title(
        "CSF at 4.5MHz for {} GHz band: $P_{{sat}}$={}pW, $P_{{opt}}=${}pW, \n $\mathcal{{L}}=${}, NEP$_{{read}}$={}aW$/\sqrt{{\mathrm{{Hz}}}}$, {}% NEP increase".format(
            lb.opt_freqs[band],
            round(psat * 1e12, 2),
            round(popt * 1e12, 2),
            bolo.loopgain[0],
    ...
    if max_power > max(req_power.flatten() * 1e9):
        ...
        vmax = max_power
    c = ax.pcolormesh(
        rbolo, rstray, req_power * 1e9, cmap="jet", shading="auto", vmax=vmax
    )
    c2 = ax.pcolormesh(
        rbolo,
        rstray,
