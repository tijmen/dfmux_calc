...
saa = d.squid(
    ...
    n_series=112,
    n_parallel=1,
    power=25e-9,  # array size and power dissipation
    snubber=5,  # if there is a snubber on the input
    t=0.1,
)  # what temperature the SAA is at [K]
...
wh = d.wire(
    ...
    rshunt=False,
    cshunt=False,
)  # if theres any resistive or capcitive shunts across the output of the SAA
# Other parasitics
stripline = 60e-12  # assumed stripline inductance
cgnd = 0.7e-12  # assumed parasitic capacitance to ground
para = d.parasitics(
    stripline, cgnd, 0
)  # stripline inductance[H], parasitic capacitance to ground[F] and R48 [Ohms]
...
bolo = d.bolo(
    1.0,  # operating impedence [Ohms] this is ignored in litebird.py
    10,  # loopgain
